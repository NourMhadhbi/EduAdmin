...
// Récupérer tous les cours avec leurs détails en utilisant la classe Cours
$cours = Cours::getAllCoursWithDetails();

// Préparer les informations sur les séances associées
$coursAvecSeances = [];
foreach ($cours as $c) {
...
<body>
    ...
    <div class="content">
            ...
            <tr>
                ...
                <td class="action-buttons">
                    <button class="btn btn-sm btn-primary edit-course-btn" 
                            ...
                            data-cours-enseignant="<?php echo htmlspecialchars($c['enseignant_nom']); ?>">
        ...
        data-has-seances="<?php echo $coursAvecSeances[$c['id']] ? 'true' : 'false'; ?>">
    <i class="fas fa-trash"></i>
</button> -->
          
                    <form method="POST" action="" style="display: inline;" 
                          onsubmit="return confirmSuppression(<?php echo $c['id']; ?>, '<?php echo htmlspecialchars($c['titre']); ?>', <?php echo $coursAvecSeances[$c['id']] ? 'true' : 'false'; ?>, <?php echo $c['nb_etudiants']; ?>)">
...
<div class="modal" id="addCourseModal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form id="addCourseForm" action="../../Controller/CoursController.php" method="POST">
...
function confirmSuppression(coursId, coursTitre, hasSeances, nbEtudiants) {
    ...
    document.getElementById('courseToDeleteName').textContent = coursTitre;
    ...
    if (hasSeances) {
        document.getElementById('seancesWarning').style.display = 'block';
    } else {
        document.getElementById('seancesWarning').style.display = 'none';
    }
    
    // Mettre à jour le formulaire de suppression
    document.getElementById('deleteCourseId').value = coursId;
    
    // Ouvrir le modal
    openModal('deleteCourseModal');
    
    // Empêcher la soumission du formulaire original
    return false;
}

// Gestionnaire d'événement pour le formulaire de suppression dans le modal
document.getElementById('deleteCourseForm').addEventListener('submit', function(e) {
    ...
    // La soumission continue normalement vers le controller
    // Le SweetAlert de succès sera affiché après le rechargement
});

// Empêcher la soumission directe des formulaires de suppression dans le tableau
document.querySelectorAll('form[onsubmit]').forEach(form => {
    form.addEventListener('submit', function(e) {
        // Cette fonction est déjà gérée par onsubmit, donc on ne fait rien de plus
    });
});
        document.getElementById('generateReportBtn').addEventListener('click', function() {
            openModal('generateReportModal');
